...
Scenario: Upload SourceSystem CDIS 
    * def corrospondenceid = read('../data/upload_VoiceOutbound.xml')
	...
	* def contract = 'VoiceOutbound'
	* def xsi_type = '&quot;#VoiceOutbound(contract)&quot'
	...
	* set corrospondenceid/Envelope/Body/Upload/metadata = contract
	Given request read('../data/upload_VoiceOutbound.xml')
	When soap action 'https://www.capita.co.uk/TfL/docstore/IServiceSoap/Upload'
	Then status 200	
	
	# Get the document id from response and replace in the response upload_VoiceOutbound_expected.xml and match response
	
	* def docid = get response /Envelope/Body/UploadResponse/UploadResult
	* def upload_VoiceOutbound_expected = read('../data/upload_DocumentInbound_expected.xml')
	* set upload_VoiceOutbound_expected/Envelope/Body/UploadResponse/UploadResult = docid	
	...
	* def TryGetByMetadataSearch = read('../data/TryGetByMetadataSearch_VoiceOutbound.xml')
	...
	* print TryGetByMetadataSearch
	...
	Given request read('../data/TryGetByMetadataSearch_VoiceOutbound.xml')
	...
	* def TryGetByMetadataSearch = read('../data/TryGetByMetadataSearch_VoiceOutbound_expected.xml')
	...
	* set TryGetByMetadataSearch/Envelope/Header/TFL-SourceSystem = contract
	And match response == read('../data/TryGetByMetadataSearch_VoiceOutbound_expected.xml')
	And print response
