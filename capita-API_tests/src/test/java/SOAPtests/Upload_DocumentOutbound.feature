...
Scenario: Upload SourceSystem CDIS 
    * def corrospondenceid = read('../data/upload_DocumentOutbound.xml')
	...
	* def contract = 'DocumentOutbound'
	* def xsi_type = '&quot;#DocumentOutbound(contract)&quot'
	...
	* set corrospondenceid/Envelope/Body/Upload/metadata = contract
	Given request read('../data/upload_DocumentOutbound.xml')
	When soap action 'https://www.capita.co.uk/TfL/docstore/IServiceSoap/Upload'
	Then status 200	
	
	# Get the document id from response and replace in the response upload_DocumentOutbound_expected.xml and match response
	
	* def docid = get response /Envelope/Body/UploadResponse/UploadResult
	* def upload_DocumentOutbound_expected = read('../data/upload_DocumentInbound_expected.xml')
	* set upload_DocumentOutbound_expected/Envelope/Body/UploadResponse/UploadResult = docid	
	...
	* def TryGetByMetadataSearch = read('../data/TryGetByMetadataSearch _DocumentOutbound.xml')
	...
	* print TryGetByMetadataSearch
	...
	Given request read('../data/TryGetByMetadataSearch _DocumentOutbound.xml')
	...
	* def TryGetByMetadataSearch = read('../data/TryGetByMetadataSearch_DocumentOutbound_expected.xml')
	...
	* set TryGetByMetadataSearch/Envelope/Header/TFL-SourceSystem = contract
	And match response == read('../data/TryGetByMetadataSearch_DocumentOutbound_expected.xml')
	And print response
