...
Scenario: Upload SourceSystem DAR Material 
    * def corrospondenceid = read('../data/upload_SupportingMaterial.xml')
	...
	* def contract = 'SupportingMaterial'
	* def xsi_type = '&quot;#SupportingMaterial(contract)&quot'
	...
	* set corrospondenceid/Envelope/Body/Upload/metadata = contract
	Given request read('../data/upload_SupportingMaterial.xml')
	When soap action 'https://www.capita.co.uk/TfL/docstore/IServiceSoap/Upload'
	Then status 200	
	
	# Get the document id from response and replace in the response upload_SupportingMaterial_expected.xml and match response
	
	* def docid = get response /Envelope/Body/UploadResponse/UploadResult
	* def upload_SupportingMaterial_expected = read('../data/upload_DocumentInbound_expected.xml')
	* set upload_SupportingMaterial_expected/Envelope/Body/UploadResponse/UploadResult = docid	
	...
	* def TryGetByMetadataSearch = read('../data/TryGetByMetadataSearch_SupportingMaterial.xml')
	...
	* print TryGetByMetadataSearch
	...
	Given request read('../data/TryGetByMetadataSearch_SupportingMaterial.xml')
	...
	* def TryGetByMetadataSearch = read('../data/TryGetByMetadataSearch_SupportingMaterial_expected.xml')
	...
	* set TryGetByMetadataSearch/Envelope/Header/TFL-SourceSystem = contract
	And match response == read('../data/TryGetByMetadataSearch_SupportingMaterial_expected.xml')
	And print response
...
Scenario: Upload SourceSystem DAR Material with repository Knowledge Base
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	And match response /Envelope/Body/Fault/detail contains 'Message: Source System Not Exist: DAR Material Repository Knowledge Base Event ID'
...
Scenario: Upload SourceSystem Amdocs with repository Enforcement Operations
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	And match response /Envelope/Body/Fault/detail contains 'Message: Source System Not Exist: Amdocs Repository Enforcement Operations Event ID'
...
Scenario: Upload SourceSystem Amdocs with repository Business Operations
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	Then status 400
    And print response	
	And match response /Envelope/Body/Fault/detail contains 'Content Type Name Not Exist in Source System: Amdocs'
	And match response /Envelope/Body/Fault/detail contains 'Message: Content Type Name Not Exist in Source System: Amdocs Event ID'
...
Scenario: Upload SourceSystem Email with repository Enforcement Operations
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	Then status 400
    And print response	
	And match response /Envelope/Body/Fault/detail contains 'Content Type Name Not Exist in Source System: Email'
	And match response /Envelope/Body/Fault/detail contains 'Message: Content Type Name Not Exist in Source System: Email Event ID'
...
Scenario: Upload SourceSystem Eptica with repository Enforcement Operations
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	Then status 400
    And print response	
	And match response /Envelope/Body/Fault/detail contains 'Content Type Name Not Exist in Source System: Eptica'
	And match response /Envelope/Body/Fault/detail contains 'Message: Content Type Name Not Exist in Source System: Eptica Event ID'
...
Scenario: Upload SourceSystem TFL Online with repository Enforcement Operations
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	Then status 400
    And print response	
	And match response /Envelope/Body/Fault/detail contains 'Content Type Name Not Exist in Source System: TFL Online'
	And match response /Envelope/Body/Fault/detail contains 'Message: Content Type Name Not Exist in Source System: TFL Online Event ID'
...
Scenario: Upload SourceSystem Taranto with repository Enforcement Operations
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	Then status 400
    And print response	
	And match response /Envelope/Body/Fault/detail contains 'Content Type Name Not Exist in Source System: Taranto'
	And match response /Envelope/Body/Fault/detail contains 'Message: Content Type Name Not Exist in Source System: Taranto Event ID'
	
Scenario: Upload SourceSystem Taranto with repository Business Operations
    * def corrospondenceid = read('../data/upload_SupportingMaterial.xml')
	* set corrospondenceid/Envelope/Body/Upload/metadata/CorrespondenceID = corrid
	* def repository = 'Business Operations'
	* def sourcesystem = 'Taranto'
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	Then status 400
    And print response	
	And match response /Envelope/Body/Fault/detail contains 'Source System Not Exist: Taranto Repository Business Operations'
	And match response /Envelope/Body/Fault/detail contains 'Message: Source System Not Exist: Taranto Repository Business Operations Event ID'	
...
Scenario: Upload SourceSystem Taranto with repository Knowledge Base
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	And match response /Envelope/Body/Fault/detail contains 'Message: Source System Not Exist: Taranto Repository Knowledge Base Event ID'	
...
Scenario: Upload SourceSystem Articles with repository Knowledge Base
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	And match response /Envelope/Body/Fault/detail contains 'Message: Content Type Name Not Exist in Source System: Articles Event ID'
...
Scenario: Upload SourceSystem Articles with repository Business Operations
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	Then status 400
    And print response	
	And match response /Envelope/Body/Fault/detail contains 'Source System Not Exist: Articles Repository Business Operation'
	And match response /Envelope/Body/Fault/detail contains 'Message: Source System Not Exist: Articles Repository Business Operations Event ID'
...
Scenario: Upload SourceSystem Articles with repository Enforcement Operations
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	And match response /Envelope/Body/Fault/detail contains 'Message: Source System Not Exist: Articles Repository Enforcement Operations Event ID'
	
Scenario: Upload SourceSystem CDIS with repository Business Operations
    * def corrospondenceid = read('../data/upload_SupportingMaterial.xml')
	* set corrospondenceid/Envelope/Body/Upload/metadata/CorrespondenceID = corrid
	* def repository = 'Business Operations'
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	Then status 400
    And print response	
	And match response /Envelope/Body/Fault/detail contains 'Content Type Name Not Exist in Source System: CDIS'
	And match response /Envelope/Body/Fault/detail contains 'Message: Content Type Name Not Exist in Source System: CDIS Event ID'
...
Scenario: Upload SourceSystem CDIS with repository Knowledge Base
	...
	* set corrospondenceid/Envelope/Body/Upload/sourceSystem = sourcesystem
	...
	And match response /Envelope/Body/Fault/detail contains 'Message: Source System Not Exist: CDIS Repository Knowledge Base Event ID'